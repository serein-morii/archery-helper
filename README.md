...
> 当前版本 **1.2.3**，更新日志见 [CHANGELOG.md](CHANGELOG.md)（插件内：设置 → 更新日志）。
...
无需手填地址：在浏览器打开过 Archery（或打开过**嵌入了 Archery 的门户页**），扩展即可通过页面特征与 Archery 接口路径特征自动识别地址并验证采用；也可随时在弹窗手动填写。候选只保存 origin，不收集任何页面内容。
