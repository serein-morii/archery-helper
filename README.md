...
> 当前版本 **1.0.9**，更新日志见 [CHANGELOG.md](CHANGELOG.md)（插件内：设置 → 更新日志）。
...
| 查询历史 / 收藏 | 最近查询记录，搜索、回填、直接执行；收藏支持**云端**（同步 Archery）与**本地**（仅存本机，可命名、分组、一键回填执行、导入导出 JSON 迁移），并支持与 Archery **云端收藏双向同步**（名称存别名、分组写入 SQL 注释，仅同步只读语句） |
| 数据导出 | 隐藏彩蛋解锁导出（CSV / Excel / JSON），触发方式就不写了，留点惊喜 |
...
| 表结构对比 | 「对比」视图两档能力：**整库对比**（多表/少表/结构差异排行）与**单表字段级对比**（选表 A/B，逐字段标出 仅A有/仅B有/定义不同/一致，B 侧自动跟随同名表）；建表语句详情为**横向并排**视图，仅本侧存在的行红色高亮 |
